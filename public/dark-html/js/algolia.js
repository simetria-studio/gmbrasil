...
const renderClearRefinements = (renderOptions, isFirstRender) => {
  ...
  document.querySelector("#clear-refinements").innerHTML = `
    <button ${
      !hasRefinements ? "disabled" : ""
    } id="clear-result" class="btn btn-primary border-0 btn-sm mr-2"><i class="ri-refresh-line mr-0"></i></button>`;
            ...
            <li class="mb-2 mr-0">
				<div class="d-flex justify-content-between">
					<div class="custom-control custom-checkbox">
					  <input type="checkbox" class="custom-control-input brand-checkbox" id="customCheck1${
              item.value
            }" data-value="${item.value}" value="" ${
            item.isRefined ? "checked" : ""
          }>
					  <label class="custom-control-label" for="customCheck1${item.value}">${
            item.label
          }</label>
					</div>
    ...
    templates: {
      item(data) {
        ...
        <a
          ...
          .map((isFilled) =>
            isFilled
              ? `<i class="ri-star-fill text-warning mr-2"></i>`
            ...
            `<li key="${index}" class="ais-Hits-item iq-product-item iq-card">
                  ...
                  <a href="">
                      <div class="h-56 d-flex align-items-center justify-content-center bg-white iq-border-radius-15">
                        <img src="${
                          item.image
                        }" align="left" alt="${instantsearch.highlight({
              attribute: "image",
              hit: item,
            })}" />
                      </div>
                  ...
                  <div class="card-body">
                      <div class="text-justify">
                        <a href="javascript:void(0)">${instantsearch
                          .highlight({ attribute: "name", hit: item })
                          .substring(0, 20)}</a>
                        <p class="font-size-12 mb-0">${instantsearch
                      ...
                      <div class="iq-product-action my-2">
                          <button type="button" class="btn btn-primary iq-waves-effect text-uppercase btn-sm addToCart" id="${
                            item.objectID
                          }">
                            <i class="ri-shopping-cart-line mr-0"></i>
                          </button>
                          <button type="button" class="btn btn-primary iq-waves-effect text-uppercase btn-sm addToWish" id="${
                            item.objectID
                          }">
                            <i class="ri-heart-line mr-0"></i>
                          </button>
                          <p class="font-size-16 font-weight-bold float-right">${
                            item.price
                          }</p>
                      </div>
                  </div>
